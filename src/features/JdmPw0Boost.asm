...
label_04e3:     L       A, 0b0h                ; 04E3 1 108 20E E5B0
                ...
                CMPB    A, #0ffh               ; 04E8 0 108 20E C6A1
                JGT     maperr 		           ; 04EA 0 108 20E C804
                CMPB    A, #000h               ; 04EC 0 108 20E C60B
                JGE     mapgood		           ; 04EE 0 108 20E CD05
                                               ; 04F0 from 04EA (DD0,108,20E)
maperr:			SC                             ; 04F0 0 108 20E 85
...
label_065e:     EXTND                          ; 065E 1 108 20E F8
				...
				;signaling to add
                RB      PSWL.5
                SJ		label_0686             ; is < dfh then we are good
                NOP
                NOP
                NOP
                NOP
...
correct0:		LB      A, #000h               ; load 0 in there
                SJ      label_0686             ; 0674 0 108 20E CB10
                                               ; 0676 from 0668 (DD0,108,20E)
label_0676:     CMPB    0a3h, #080h            ; euro pw0 replace temp check with nops
                JLT     label_0682             ; subtracts
                CMPB    0f9h, #00fh            ; 067C 0 108 20E C5F9C00F
                JLT     correct0	           ; does nothing
                                               ; 0682 from 067A (DD0,108,20E)
label_0682:     SB      PSWL.5					;subtract
...
label_06c5:     LB      A, #000h                ; boost code change
...
label_06d4:     LB      A, #000h                ; boost code change
...
label_0a99:     LB      A, r7                  ; 0A99 0 108 13D 7F
                ...
                ;boost code changes
                MOVB    r6, 0b6h               ; 0AA4 0 108 13D C5B54E
                ...
                MOVB    r6, 0b6h               ; 0AC0 0 108 13D C5B54E
...
label_16dd:     MOVB    WDT, #03ch             ; 16DD 0 ??? ??? C511983C
				...
				;datalogging change
                MOV     SSP, #0025bh           ; 16E1 0 ??? ??? A0986402
...
label_1721:     CLRB    PRPHF                  ; 1721 0 080 ??? C51215
                ...
                MOVB    STCON, #03ch           ; 1742 0 080 ??? C5509831
                MOVB    SRCON, #02ch           ; 1746 0 080 ??? C5549821
                MOVB    STTM, #0f3h            ; 174A 0 080 ??? C54898FC
                MOVB    STTMR, #0f3h           ; 174E 0 080 ??? C54998FC
...
label_2044:     CMP     SSP, #0025bh           ; 2044 1 080 132 A0C06402
...
label_2096:     LC      A, [X1]                ; 2096 0 080 220 90A8
                ...
                SJ      label_20c4             ; boost code change: checksum
...
label_2220:     JBR     off(0001fh).1, label_223a ; 2220 0 080 213 D91F17
				...
				;boost code changes
				;mechanical map code action
                LB      A, #000h               ; was b6h
                ;SUBB    A, #000h               ; was b1h
                SJ     label_222d             ; jump to no code
                NOP
                NOP
                NOP
...
label_222d:     CMPB    A, #002h               ; 222D 0 080 213 C602
                ...
                SJ      label_223a             ; 2234 0 080 213 CB05
...
label_2cb3:     CLR     A
                ...
                LCB     A, 00165h[X1]          ; 2CBE 0 108 20E 90ABFF00
                MOV     DP, A                  ; 2CC2 0 108 20E 52
                CMPCB   A, 00166h[X1]          ; 2CC3 0 108 20E 90AF0001
...
label_2f39:     LB      A, ADCR2H              ; boost code change: o2 sensor disable
...
launch2:		MB      C, P2.4      ;do the line we replaced
				RT
;*************************************************************
; stores the fuel row number into 17ch and ign row into 17dh for logging.
...
storefuel:		LB		A, r0
				...
				LB		A, r7
				STB		A, off(001ddh)			; fuel row inerp
...
storeign:       STB		A, off(0017dh)			; store the ignition row into 17d for me
				...
				LB		A, r7
				STB		A, off(001deh)			; ign row inerp
...
logging_table:  DB  098h,000h ;10 water temp
                ...
                DB  0ddh,001h ;29 fuel row interpolation
                DB  0deh,001h ;2a ignition row interpolation
...
; try1 - solid cel. poop.
;
; test1 - has no codes disable stuff, rearranged, stupid recognizer taken out
;
