...
int_serial_rx_vec:        DW  int_serial_rx    ; 000A 0035
...
code_start:     DB  008h,00Eh,00Eh,000h,0E5h,0CEh,0D5h,01Ah ; 0038
                ...
                DB  0E5h,0CCh,0A2h,008h,0D5h,01Ah,002h,0E5h ; 0060
                DB  0CEh,0D5h,01Ah,0A2h,018h,042h,055h,0FAh ; 0068
                DB  0C5h,056h,00Bh,0C9h,002h,086h,001h,0C5h ; 0070
                DB  056h,00Ah,0C9h,002h,086h,002h,0D5h,007h ; 0078
                DB  0F5h,055h,052h,0F2h,0D5h,051h,065h,052h ; 0080
                DB  0E5h,0CCh,0A2h,008h,0D5h,01Ah,002h ; 0088
                                               ; 008F from 0006 (DD0,???,???)
...
label_04ca:     STB     A, 0a7h                ; 04CA 0 108 20E D5A7
                JBS     off(00130h).2, mapstore ; 04CC 0 108 20E EA3028

;***********************************************************************

                ;map calc
                L       A, 0b0h                ; 04EC 1 108 20E E5B0
                SWAP                           ; 04EE 1 108 20E 83
                LB      A, ACC                 ; dd = 0
                CMPB    A, #0ffh               ; ffh for boost
                JGT     maperror               ; err
                CMPB    A, #00bh               ;
                JGE     mapgood		           ; we are within range

maperror:		SC                             ; 04F9 0 108 20E 85
                SJ      label_04f8             ; 04FA 0 108 20E CB1E
...
set_column:		ST      A, er1					;store the calced val
				...
				LB      A, ACC					;
				STB		A, 0b6h					;store the column for later

				CAL		calcb4h

mapstore:		RC                             ; reset carry because we dont want a map code


				;error if carry set
                                               ; 04F8 from 04D6 (DD0,108,20E)
                                               ; 04F8 from 04DB (DD0,108,20E)
label_04f8:     MB      off(0012ch).0, C       ; map sensor code set if C == 1

                JBS     off(00130h).4, label_0503 ; 04FB 0 108 20E EC3005
                JLT     label_0503             ; if c==1 then use TPS
                JBR     off(00130h).2, label_0509 ; 0500 0 108 20E DA3006

                ;tps usage if map error
...
label_0655:     EXTND                          ; 0655 1 108 20E F8
                ...
                ;signal to add...
                RB      PSWL.5					; add
                SJ      label_067d              ; jump to store correction
                NOP
                NOP
                NOP
...
label_066d:     JBS     off(00128h).1, label_0679 ; 066D 0 108 20E E92809
                ...
                JLT     correct0               ; 0677 0 108 20E CA04
                                               ; 0679 from 066D (DD0,108,20E)

				;signal to subtract
label_0679:     SB      PSWL.5					;subtract
				NOP
				NOP
				NOP
...
label_06bc:     LB      A, #000h               ; 06BC 0 108 20E F5B4
...
label_06cb:     LB      A, #000h               ; 06CB 0 108 20E F5B4
...
label_06d3:     ST      A, off(00150h)         ; 06D3 1 108 20E D450
                ...
                ;aaaand we're into the table stuff...
                ;ignition tables...
...
label_06df:     MOVB    r6, 0b6h               ; move the column in
                LB      A, 0a7h                ; 06E0 0 108 20E F5A7

                RC                             ; 06E2 0 108 20E 95
                MOV     X1, #ignitionmapv      ; 06E3 0 108 20E 60E53C
                ;MOV     X2, #revscalar_igv     ; 06E6 0 108 20E 61C63B
                MOV     X2, #03bc6h
                JBS     off(00129h).7, label_06f7 ; if vtec then use above pointers
                LB      A, 0a6h                ; 06EC 0 108 20E F5A6
                MB      C, off(00129h).1       ; 06EE 0 108 20E C42929
                MOV     X1, #ignitionmap       ; 06F1 0 108 20E 60E63B
                ;MOV     X2, #revscalar_ig      ; 06F4 0 108 20E 61B63B
                MOV     X2, #03bb6h
...
label_06f7:     STB     A, r7                  ; 06F7 0 108 20E 8F
                ...
                CAL     label_2b3f             ; interp

                MOVB    off(00138h), A         ; put it in 138h
...
label_0988:     MOVB    r6, 0b6h               ; move in column
                MOVB    r7, 0a6h               ; non vtec rpm
                MOV     X1, #fuelmap           ; 098E 0 108 13D 60E43D
                ;MOV     X2, #revscalar_fu      ; 0991 0 108 13D 61D63B
                MOV     X2, #03bd6h
               ...
               ; MB      off(00129h).2, C       ; 0997 0 108 13D C4293A
                ...
                MOVB    r6, 0b6h               ; move in column
                MOVB    r7, 0a7h               ; vtec rpm
                MOV     X1, #fuelmapv          ; 09AA 0 108 13D 60F23E
                ;MOV     X2, #revscalar_fuv     ; 09AD 0 108 13D 61E63B
                MOV     X2, #03be6h
...
label_1678:     MOVB    WDT, #03ch             ; 1678 0 ??? ??? C511983C
                MOV     SSP, #0025bh           ; 167C 0 ??? ??? A0985B02
...
label_208f:     JBR     off(0001fh).1, label_20a9 ; 208F 0 080 213 D91F17
				...
				;uses b1h and b6h
                LB      A, #000h               ; 2092 0 080 213 F5B6
                SJ      label_209c             ; 2097 0 080 213 CD03
                NOP
                NOP
                NOP
                NOP
...
label_209c:     CMPB    A, #002h               ; 209C 0 080 213 C602
                RB      0fdh.6                 ; 20A0 0 080 213 C5FD1E
                SJ      label_20a9             ; 20A3 0 080 213 CB05
                                               ; 20A5 from 209E (DD0,080,213)
label_20a5:     LB      A, off(000e8h)         ; skipped
                JEQ     label_20aa             ; skipped
...
label_2b3f:     CLR     A                      ; 2B3F 1 108 20E F9
                ...
                LCB     A, 00165h[X1]          ; 2B4A 0 108 20E 90ABFF00
                MOV     DP, A                  ; 2B4E 0 108 20E 52
                CMPCB   A, 00166h[X1]          ; 2B4F 0 108 20E 90AF0001
...
label_2dc5:     LB      A, 0a1h                ; secondary o2 disable...
...
label_2f22:     INC     X1                     ; 2F22 0 080 1C7 70
                ...
                RT                             ; 2F27 0 080 1C7 01
                                               ; 2F28 from 18C5 (DD0,080,1B1)
                                               ; 2F28 from 2F31 (DD0,080,1B2)
                                               ; 2F28 from 1DF8 (DD0,080,1D4)
                                               ; 2F28 from 1E8A (DD0,080,1C7)
...
;b5h = the correction
;b4h = calulated map value.
;b6h = col val from map scalar

;on return
;b6h = corrected column value: 0 <= b6h <= colsize-2
;b5h = corrected nibble for table inerpolation

correctcol:		MOV		X1, #colsize
				CLR		A				;clear AH
				LB		A, 0b5h			;load the correction
				...
				;****************else we subtract
				LB		A, 0b4h			; these 2 lines get the correction
				ANDB	A, #00fh		; least sig nibble
				ADDB	A, #0f0h		; make A <= f0h
				SUBB	A, 0b5h			; subtract the correction
...
st_sub_cor:		STB		A, 0b5h			; correct nibble for interpolation
...
ccorresume2:	STB		A, 0b6h
				SJ		ccordone

				;*****add correction
addcol:			LB		A, 0b4h			; these 2 lines get the correction
				ANDB	A, #00fh		; least sig nibble
				ADDB	A, 0b5h			; add the correction
				JLT		overcorr		; if carry then its over ffh
				SJ		ccorresume		; else we are good

overcorr:		LB		A, #0ffh		; set to dfh if the correction was huge.

ccorresume:		STB		A, r4			; store the corrected nibble into r4
				SRL		A				;
				SRL		A				;
				SRL		A				;
				SRL		A				; shift right x4 to get the column correction
				ADDB	A, 0b6h			; add that shit to b6h
				STB		A, 0b6h			; store it in b6h
...
st_add_cor:		STB		A, 0b5h
				SJ		ccordone		; we're done adding
				;*********************

nocorr:			LB		A, 0b4h			; put b4h
				STB		A, 0b5h			; in b5h
			;	LB		A, 0b6h			; put b6h
			;	STB		A, 0b6h			; in b6h

				;should be: if(maxcol >= b6h) then return else b6h = maxcol;
ccordone:		LCB		A, [X1]			; load column size
				SUBB	A, #002h		; get highest column
				CMPB	A, 0b6h			; compre to the currently stored column
				JGE		ccorreturn		; if(maxcol >= b6h) then return
				STB		A, 0b6h			; else we store the highest column value
...
;
;                STB		A, off(0017dh)			; store multiplier into 17dh
;
;                INC		X1						; X1++
...
;*****************************************************
; this function calculates the high nibble of b4h ram based on current row and max row.
; Its a percentage thing. If there are 30 columns and the current column is 15 then we are half way
; through the map and b5h (the map image location) will show the rest of the stock code a false
...
;				LB		A, ACC				; dd = 0
;				DIVB						; AL = (current column*dh)/max column == b4h high nibble
;				SLLB	A
;				SLLB	A
;				SLLB	A
;				SLLB	A
...
;
;				CMPB	A, #0dfh			;
;				JLE		calcb4hrt			; if calculated b4h value <= #dfh then return
...
;****************************************************************************
				...
				;take this out when the file grows...

                ORG	036FCh
                DB	000h,0FFh,059h,0F5h ; 36FC
                DB  059h,0E8h,04Dh,0BAh,048h,087h,047h,030h ; 3700
                DB  043h,028h,040h,000h,040h,0FFh,078h,0F5h ; 3708
                DB  078h,0E1h,06Ch,0BAh,063h,087h,05Dh,030h ; 3710
                DB  04Bh,028h,040h,000h,040h,0FFh,069h,0F5h ; 3718
                DB  069h,0E1h,05Ah,0BAh,057h,087h,056h,030h ; 3720
                DB  04Bh,028h,040h,000h,040h,0FFh,05Eh,0F5h ; 3728
                DB  05Eh,0E1h,05Bh,0BAh,056h,087h,04Eh,030h ; 3730
                DB  045h,028h,040h,000h,040h,0DFh,0DFh,051h ; 3738
                DB  051h,0FFh,05Ah,0E0h,044h,0C0h,02Ah,0A0h ; 3740
                DB  00Fh,080h,009h,050h,000h,000h,000h,092h ; 3748
                DB  000h,05Bh,047h,0FFh,090h,0E9h,0A0h,0CAh ; 3750
                DB  0B0h,0C5h,0EEh,000h,0EEh,0FFh,0C0h,0E9h ; 3758
                DB  0C0h,0CAh,0B0h,0C5h,0EEh,000h,0EEh,0FFh ; 3760
                DB  0E2h,004h,0E0h,050h,007h,0C0h,086h,007h ; 3768
                DB  0A0h,086h,007h,000h,0D6h,006h,0FFh,0D6h ; 3770
                DB  006h,0E8h,066h,008h,0C0h,086h,007h,0A0h ; 3778
                DB  086h,007h,000h,0D6h,006h,005h,00Dh,013h ; 3780
                DB  018h,060h,000h,0C0h,001h,0C0h,001h,020h ; 3788
                DB  000h,000h,003h,020h,000h,0E0h,000h,008h ; 3790
                DB  007h,0C5h,007h,0E0h,000h,07Dh,007h,02Fh ; 3798
                DB  008h,0E0h,000h,008h,007h,0C5h,007h,0E0h ; 37A0
                DB  000h,046h,008h,025h,009h,080h,000h,040h ; 37A8
                DB  000h,0FFh,025h,0E9h,025h,0D7h,025h,0C6h ; 3900
                DB  025h,097h,024h,046h,00Eh,030h,000h,000h ; 3908
                DB  000h,008h,001h,004h,001h,0BEh,02Eh,07Ah ; 3910
                DB  000h,0BEh,000h,094h,000h,077h,000h,064h ; 3918
                DB  005h,00Fh,003h,005h,032h,032h,05Fh,001h ; 3920
                DB  0E7h,000h,0FAh,000h,00Ch,001h,05Fh,001h ; 3928
                DB  0EDh,000h,001h,001h,014h,001h,044h,0A9h ; 3930
                DB  032h,062h,0FFh,019h,0C6h,019h,094h,019h ; 3938
                DB  086h,000h,000h,000h,0FFh,097h,0D0h,091h ; 3940
                DB  07Ah,070h,044h,054h,02Eh,043h,000h,043h ; 3948
                DB  094h,000h,05Bh,00Bh,018h,018h,025h,0FFh ; 3950
                DB  0D7h,0C6h,098h,069h,04Ah,000h,000h,030h ; 3958
                DB  080h,012h,05Ah,093h,080h,051h,05Ah,0FFh ; 3960
                DB  08Ah,066h,0F5h,08Ah,066h,0E1h,0EBh,041h ; 3968
                DB  0BAh,03Ah,020h,087h,0A6h,00Eh,028h,0E7h ; 3970
                DB  008h,000h,0E7h,008h,0FFh,08Ah,066h,0F5h ; 3978
                DB  08Ah,066h,0E1h,0EBh,041h,0BAh,03Ah,020h ; 3980
                DB  087h,0A6h,00Eh,028h,0E7h,008h,000h,0E7h ; 3988
                DB  008h,0FFh,0FFh,01Bh,0ABh,000h,015h,08Eh ; 3990
                DB  000h,011h,072h,000h,008h,063h,000h,00Ch ; 3998
                DB  055h,000h,000h,000h,000h,000h,0FFh,000h ; 39A0
                DB  008h,0E9h,000h,017h,0D8h,000h,017h,0CAh ; 39A8
                DB  000h,010h,0A9h,000h,00Eh,090h,000h,000h ; 39B0
                DB  000h,000h,000h,0FFh,040h,004h,0F8h,040h ; 39B8
                DB  004h,0F8h,040h,004h,08Eh,080h,002h,078h ; 39C0
                DB  000h,000h,000h,000h,000h,0F1h,000h,000h ; 39C8
                DB  028h,000h,000h,0FFh,091h,0D0h,091h,07Ah ; 39D0
                DB  077h,044h,057h,02Eh,044h,000h,044h,0FFh ; 39D8
                DB  012h,004h,0A1h,012h,004h,07Ah,0E2h,004h ; 39E0
                DB  044h,0A8h,006h,02Eh,0C4h,009h,000h,0C4h ; 39E8
                DB  009h,0C4h,009h,064h,009h,00Bh,009h,0FFh ; 39F0
                DB  000h,00Ah,0F2h,000h,00Ah,0E1h,000h,006h ; 39F8
                DB  0C6h,000h,006h,087h,000h,00Eh,065h,000h ; 3A00
                DB  00Ah,044h,000h,006h,02Eh,000h,000h,000h ; 3A08
                DB  000h,000h,080h,000h,006h,028h,080h,008h ; 3A10
                DB  080h,080h,006h,028h,000h,009h,000h,003h ; 3A18
                DB  040h,000h,000h,003h,040h,000h,001h,000h ; 3A20
                DB  000h,000h,000h,000h,000h,001h,000h,000h ; 3A28
                DB  0A0h,001h,0FFh,000h,010h,0A9h,000h,00Eh ; 3A30
                DB  097h,000h,00Bh,086h,000h,008h,069h,000h ; 3A38
                DB  005h,054h,000h,000h,000h,000h,000h,010h ; 3A40
                DB  000h,008h,002h,000h,000h,0FFh,020h,000h ; 3A48
                DB  0F5h,020h,000h,0E1h,012h,000h,0D7h,01Bh ; 3A50
                DB  000h,0FFh,000h,012h,0F2h,000h,012h,0D0h ; 3A58
                DB  000h,00Ah,0A1h,000h,006h,056h,000h,004h ; 3A60
                DB  044h,080h,004h,02Eh,000h,006h,020h,000h ; 3A68
                DB  009h,000h,000h,009h,030h,000h,028h,000h ; 3A70
                DB  018h,000h,000h,00Ch,000h,001h,030h,000h ; 3A78
                DB  028h,000h,018h,000h,000h,010h,040h,002h ; 3A80
                DB  093h,080h,061h,09Dh,0FFh,0C0h,000h,0E0h ; 3A88
                DB  0C0h,000h,0A1h,01Ah,000h,02Eh,007h,000h ; 3A90
                DB  000h,007h,000h,0FFh,02Eh,000h,0A1h,02Eh ; 3A98
                DB  000h,057h,01Ah,000h,02Eh,018h,000h,000h ; 3AA0
                DB  018h,000h,0FFh,0FFh,000h,080h,0FFh,01Bh ; 3AA8
                DB  000h,078h,060h,016h,010h,047h,0C8h,010h ; 3AB0
                DB  0E0h,03Dh,030h,00Bh,0B0h,034h,000h,002h ; 3AB8
                DB  080h,01Fh,000h,000h,0F0h,017h,0FFh,0FFh ; 3AC0
                DB  08Fh,042h,000h,0FEh,08Fh,042h,000h,0FBh ; 3AC8
                DB  0AEh,067h,000h,0F6h,0C2h,075h,000h,0F0h ; 3AD0
                DB  000h,080h,000h,0E9h,01Eh,085h,000h,0E0h ; 3AD8
                DB  000h,080h,000h,000h,000h,080h,0E0h,033h ; 3AE0
                DB  0A9h,051h,019h,097h,0D8h,0DDh,0E5h,0E9h ; 3AE8
                DB  0FFh,076h,007h,0F0h,076h,007h,0E0h,076h ; 3AF0
                DB  007h,0D9h,026h,007h,0D4h,05Ch,008h,0CFh ; 3AF8
                DB  02Ah,008h,000h,02Ah,008h,0E7h,008h,023h ; 3B00
                DB  00Dh,09Ch,017h,03Bh,033h,0EBh,041h,030h ; 3B08
                DB  001h,038h,001h,09Fh,001h,08Ah,002h,024h ; 3B10
                DB  003h,068h,0D0h,020h,060h,0A2h,033h,073h ; 3B18
                DB  02Ah,000h,008h,0FFh,040h,028h,06Eh,000h ; 3B20
                DB  014h,00Fh,00Fh,00Fh,02Dh,00Fh,006h,02Dh ; 3B28
                DB  00Fh,02Dh,04Bh,02Dh,0FFh,02Dh,02Dh,0FFh ; 3B30
                DB  0FFh,003h,006h,007h,005h,00Dh,015h,016h ; 3B38
                DB  00Ah,00Eh,008h,011h,000h,017h,018h,001h ; 3B40
                DB  002h,004h,008h,009h,00Fh,004h,008h,009h ; 3B48
                DB  000h,000h,000h,000h,000h,000h,000h,077h ; 3B50
                DB  011h,0EEh,022h,077h,022h,0DDh,044h,0FFh ; 3B58
                DB  0FFh,0EEh,044h,077h,044h,0BBh,088h,0BBh ; 3B60
                DB  011h,0FFh,0FFh,0BBh,022h,0DDh,088h,0DDh ; 3B68
                DB  011h,0EEh,088h,000h,000h,0C7h,000h,02Dh ; 3B70
                DB  02Dh,007h,006h,019h,019h,019h,000h,0B3h ; 3B78
                DB  00Bh,0B3h,00Bh,0FFh,04Bh,096h,096h,01Ch ; 3B80
                DB  002h,005h,00Ah,00Ah,00Dh,00Dh,000h,000h ; 3B88
                DB  072h,081h,05Dh,053h,086h,05Dh,079h,096h ; 3E00
                DB  05Ah,068h,073h,080h,08Eh,04Eh,05Eh,072h ; 3E08
                DB  07Ch,05Fh,057h,088h,061h,07Ch,097h,05Ch ; 3E10
                DB  068h,073h,080h,08Eh,04Eh,05Eh,074h,07Dh ; 3E18
                DB  061h,059h,090h,064h,07Eh,09Bh,05Dh,06Ah ; 3E20
                DB  076h,083h,091h,050h,061h,076h,080h,064h ; 3E28
                DB  05Eh,09Fh,06Bh,086h,0A1h,05Fh,06Dh,07Ah ; 3E30
                DB  088h,095h,054h,064h,079h,083h,06Eh,065h ; 3E38
                DB  0A0h,06Dh,087h,0A2h,061h,070h,07Eh,089h ; 3E40
                DB  09Bh,054h,065h,077h,081h,076h,06Ch,0A7h ; 3E48
                DB  06Fh,08Ch,0A9h,064h,073h,081h,08Fh,09Eh ; 3E50
                DB  057h,066h,078h,080h,07Bh,06Fh,0ADh,072h ; 3E58
                DB  090h,0ABh,064h,075h,080h,08Fh,09Eh,057h ; 3E60
                DB  068h,07Bh,08Ah,07Bh,06Ah,0A6h,071h,08Ch ; 3E68
                DB  0AAh,066h,074h,085h,095h,0A5h,05Ah,06Bh ; 3E70
                DB  081h,091h,082h,073h,0B1h,075h,094h,0B2h ; 3E78
                DB  069h,079h,086h,097h,0A6h,05Ch,06Ch,081h ; 3E80
                DB  095h,07Fh,070h,0AFh,075h,092h,0B4h,069h ; 3E88
                DB  07Ah,089h,09Ah,0A9h,05Dh,06Dh,085h,096h ; 3E90
                DB  06Ch,063h,09Ch,06Eh,08Ah,0AAh,064h,075h ; 3E98
                DB  082h,092h,0A3h,05Ah,06Ch,080h,094h,082h ; 3EA0
                DB  077h,0B9h,07Dh,09Eh,0C3h,071h,07Fh,095h ; 3EA8
                DB  0A6h,0BAh,064h,07Ah,082h,093h,0BEh,09Ah ; 3EB0
                DB  0E5h,097h,0BBh,0DFh,082h,093h,0A9h,0BCh ; 3EB8
                DB  0D0h,072h,085h,099h,0ADh,0B4h,092h,0DBh ; 3EC0
                DB  08Fh,0BAh,0DDh,081h,095h,0AAh,0BCh,0CFh ; 3EC8
                DB  072h,086h,09Ah,0AEh,096h,08Ah,0C9h,092h ; 3ED0
                DB  0BEh,0E3h,087h,0A0h,0B5h,0C7h,0DAh,074h ; 3ED8
                DB  08Bh,09Ah,0AAh,000h,001h,001h,002h,002h ; 3EE0
                DB  002h,003h,003h,003h,003h,003h,004h,004h ; 3EE8
                DB  004h,004h,03Fh,035h,066h,050h,066h,07Ch ; 3EF0
                DB  04Bh,059h,066h,075h,084h,049h,05Bh,06Dh ; 3EF8
                DB  07Fh,03Fh,035h,066h,050h,066h,07Ch,04Bh ; 3F00
                DB  059h,066h,075h,084h,049h,05Bh,06Dh,07Fh ; 3F08
                DB  03Fh,035h,066h,050h,066h,07Ch,04Bh,059h ; 3F10
                DB  066h,075h,084h,049h,05Bh,06Dh,07Fh,03Fh ; 3F18
                DB  035h,066h,050h,066h,07Ch,04Bh,059h,066h ; 3F20
                DB  075h,084h,049h,05Bh,06Dh,07Fh,03Fh,035h ; 3F28
                DB  066h,050h,066h,07Ch,04Bh,059h,066h,075h ; 3F30
                DB  084h,049h,05Bh,06Dh,07Fh,04Eh,044h,060h ; 3F38
                DB  045h,05Fh,07Eh,04Fh,05Dh,06Dh,07Fh,08Dh ; 3F40
                DB  050h,062h,077h,08Ch,05Fh,050h,084h,063h ; 3F48
                DB  080h,0A1h,061h,071h,07Fh,091h,0A2h,05Ah ; 3F50
                DB  06Fh,083h,097h,049h,03Dh,06Dh,04Bh,067h ; 3F58
                DB  082h,050h,060h,070h,081h,092h,052h,066h ; 3F60
                DB  080h,096h,055h,048h,077h,051h,06Bh,088h ; 3F68
                DB  053h,064h,074h,087h,09Bh,05Bh,072h,083h ; 3F70
                DB  094h,067h,062h,0A4h,069h,089h,0ADh,067h ; 3F78
                DB  07Ah,08Dh,09Fh,0B3h,068h,07Dh,08Ah,098h ; 3F80
                DB  08Fh,07Ah,0C1h,084h,0A9h,0CFh,07Ah,091h ; 3F88
                DB  0A4h,0BBh,0CFh,071h,086h,097h,0A8h,0A3h ; 3F90
                DB  08Ah,0D5h,091h,0B5h,0DCh,081h,096h,0ABh ; 3F98
                DB  0C1h,0D5h,074h,087h,09Ah,0ADh,0AFh,096h ; 3FA0
                DB  0DFh,099h,0C2h,0E9h,08Bh,0A2h,0B7h,0C9h ; 3FA8
                DB  0DFh,077h,08Bh,09Fh,0B3h,0B2h,097h,0DEh ; 3FB0
                DB  09Ch,0C6h,0F5h,091h,0A9h,0BFh,0D4h,0E8h ; 3FB8
                DB  080h,096h,0A8h,0BAh,085h,081h,0BDh,08Eh ; 3FC0
                DB  0C2h,0FCh,096h,0AFh,0C5h,0DDh,0F5h,07Eh ; 3FC8
                DB  087h,0A9h,0C4h,085h,081h,0BDh,08Eh,0C2h ; 3FD0
                DB  0FCh,096h,0AFh,0C5h,0DDh,0F5h,07Eh,087h ; 3FD8
                DB  0A9h,0C4h,085h,081h,0BDh,08Eh,0C2h,0FCh ; 3FE0
                DB  096h,0AFh,0C5h,0DDh,0F5h,07Eh,087h,0A9h ; 3FE8
                DB  0C4h,000h,001h,001h,002h,002h,002h,003h ; 3FF0
                DB  003h,003h,003h,003h,004h,004h,004h,004h ; 3FF8
...
mapscalar:		DB	0ffh,03fh,001h,0feh,03fh,001h,0f0h,030h,001h,0e0h,020h,001h
				...
				DB	020h,010h,000h,000h,000h,000h,000h,000h
...
logging_table:  DB  098h,000h ;10 water temp
                DB  099h,000h ;11 IAT
                DB  0b6h,000h ;12 original map column
                ...
                DB  0deh,000h ;2a ignition row interpolation
...
;*******************versions
; try1 - full dev rom.
;	- very rich/lean fluctuation. did not test boost.
;
; try2 - stock interpolation function, no correction.
;	- no fluctuation, correction function good
;
; try3 - stock interp, with correction.
;   - no fluctuation. Hit boost but "revlimiter" type behavior.
;	  could be from calcb4h error...
;
; try4 - correction val b5h capped at #dfh.
;	- it worked!!! no boost cut. fullboost is good.
;	- martha focking boost cut sometimes. probably because there is no cap in
;	  correction subtraction.
;
; try5 - try 4 but using static colsize value in calcb4h (which simplifies things a lot)
;	- not tested
; try6 - more simplified calcb4h
;	- worked!!
;
; try7 - try4 with cap on b5h in sub and add. This should be the one!
;   - revlimit again. *sigh*
;
; try8 - try6 with no 2nd o2.
;
